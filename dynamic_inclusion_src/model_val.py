...
class Eikonal(torch.nn.Module):
    def __init__(self, args, alpha, graph, dev, mask, lamb, nowgts):
        ...
        self.graph = graph
        self.dev = dev
        if self.args.saved_front :
            self.distances = torch.load(f"wgts{int(self.args.time)}.pt").to(self.dev)
        else:
...
class EikonalBlock(torch.nn.Module):
    def __init__(self, args, odefunc,t, dev, **kwargs):
...
class Net(torch.nn.Module):
    ...
    def __init__(self, args, alpha, graph,  dev, **kwargs):
        ...
        self.m1 = Linear(kwargs["inp"],int(self.args.hs1),weight_initializer='kaiming_uniform')
        self.m2 = Linear(int(self.args.hs1),int(self.args.hs2),weight_initializer='kaiming_uniform')
        self.m3 = Linear(int(self.args.hs2),kwargs["out"], weight_initializer='kaiming_uniform')
        self.dropout1 = torch.nn.Dropout(kwargs["alpha1"])
        self.dropout2 = torch.nn.Dropout(kwargs["alpha2"])
        self.rel = ReLU()
        # self.rel = ELU()
        if self.args.saved_front:
            self.front_initial = torch.load(f"init_dist{int(self.args.time)}.pt").to(self.args.dev)
            self.mask = torch.where(get_front_eikonal(self.graph.y, self.graph.sum_mask)==0, False, True)
        else:
        ...
        # self.mask = torch.where(get_front_eikonal(self.graph.y, self.graph.train_mask)==0, False, True)
        self.time = (torch.linspace(0,args.time,2)).to(args.dev)
        self.sig = Sigmoid()
        self.eikonalblock = EikonalBlock(self.args, Eikonal(self.args, self.alpha, self.graph, self.dev, self.mask, kwargs["lamb"], kwargs["nowgts"]), t=self.time, dev=self.dev, **dict(rtol=kwargs["rtol"]))
    ...
    def forward(self,x):
        ...
        # x = self.rel(x)
        x = torch.abs(x)
        s_vec = self.sig(self.s_vec.weight)
        if not self.run_pde:
            ...
            if self.args.saved_front :
                self.fzero = (self.front_initial.T * self.mask).T
            else:
    ...
    def graph_out2(self):
        ...
        # testf = torch.cat([test1,test2,test3],dim=1)
        # testf = test1
    ...
    def save_front(self):
        ...
        torch.save((-1.0)* out_front.cpu(), f"init_dist{int(self.args.time)}.pt")
        torch.save(new_model.eikonalblock.odefunc.distances.cpu(), f"wgts{int(self.args.time)}.pt")
        return
    ...
    def save_dist(self):
        ...
        test2 = out.T/(test.view(1,-1)) 
        torch.save((-1.0) * test2.cpu(), f"test{int(self.args.time)}.pt")
        print(f"learned eikonal distances t={int(self.args.time)}")
        return
    ...
    def fit(self):
        ...
        for epoch in range(1, 5000):
            ...
            # if val_loss < best_val_loss:
                ...
                torch.save(self, 'downstream.pth')
            else:
                patience_counter -= 1
